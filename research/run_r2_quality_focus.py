...
class C:bo:int;eff:float;cl:float;rv:float;rsi:float;adx:str;harvest:float
...
def adxok(ind,i,mode):
 if i<=0 or ind[i]['adx'] is None or ind[i-1]['adx'] is None:return False
 v=float(ind[i]['adx']);pv=float(ind[i-1]['adx']);thr=17 if mode=='17R' else 20
 return v>=thr and v>pv

def run(b,ind,c,p):
 q=0.;av=0.;base=None;a0=None;a1=a2=hv=False;a1i=a2i=None;cur=None;rs=[]
 for i,(bar,r) in enumerate(zip(b,ind)):
  inside=p[0]<=bar.trade_date<=p[1];u,fr=fresh(b,i,c.bo);l10=lo(b,i,10);l55=lo(b,i,55);atr=float(r['atr']) if r['atr'] is not None else None;eff=float(r['efficiency']) if r['efficiency'] is not None else None;rsi=float(r['rsi']) if r['rsi'] is not None else None;cl=(bar.close-bar.low)/(bar.high-bar.low) if bar.high>bar.low else .5;rv=float(r.get('relative_volume') or 0)
  mat=i>=2 and int(ind[i-1]['trend_state'])==1 and int(ind[i-2]['trend_state'])==1
  normal=inside and q==0 and fr and not bool(r['is_earnings_up_gap']) and int(r['trend_state'])==1 and mat and bar.close>=25 and eff is not None and eff>=c.eff and rsi is not None and rsi<=c.rsi and adxok(ind,i,c.adx) and cl>=c.cl and rv>=c.rv
  ...
  adx=float(r['adx']) if r['adx'] is not None else None;tok=int(r['trend_state'])==1 and adx is not None and adx>=17;ad1=q>0 and not a1 and base is not None and a0 is not None and tok and bar.close>=base+a0
  ...
  if har:hv=True
  ex=inside and q>0 and l55 is not None and bar.close<l55
   ...
   for cl in (0,.65,.75):
    for rv in (0,1.2):
     for rsi in (75,80):
      for ax in ('17R','20R'):
       for hv in (0,5):
        c=C(bo,eff,cl,rv,rsi,ax,hv);tr=ev(d,ss,c,TR)
        if tr['trades']>=55 and tr['exp']>0 and (tr['pf'] or 0)>1:
         va=ev(d,ss,c,VA)
         if va['trades']>=12 and va['exp']>0:
          ho=ev(d,hh,c,FU);st=ev(d,list(d),c,ST);arr.append({'c':asdict(c),'tr':tr,'va':va,'ho':ho,'st':st,'strict':ho['avgw']>=2 and (ho['pf'] or 0)>=2 and ho['exp']>0 and st['avgw']>=2 and (st['pf'] or 0)>=2 and st['exp']>0})
 arr.sort(key=lambda x:(x['strict'],min(x['ho']['pf'] or 0,x['st']['pf'] or 0),min(x['ho']['exp'],x['st']['exp'])),reverse=True);strict=[x for x in arr if x['strict']];res={'n':len(d),'search':len(ss),'holdout':len(hh),'strict_count':len(strict),'champion':strict[0] if strict else(arr[0] if arr else None),'top':arr[:15]};Path('r2-quality-focus.json').write_text(json.dumps(res,indent=2));print(json.dumps(res,indent=2))
