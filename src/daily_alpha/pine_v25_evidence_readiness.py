...
from .pine_v24_evidence_readiness import EvidenceArtifactState
from .pine_v25_historical_reference import build_historical_v25_reference
...
class ForwardV25EvidenceReadiness:
    """Machine-readable prerequisites for trusted SH25 forward replay."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.trading_authorized or self.live_trading_enabled:
            raise ValueError("SH25 evidence readiness cannot authorize trading")
        if self.ready_for_locked_replay and not self.reference_signal_ids:
            raise ValueError("locked SH25 replay cannot be ready without genuine reference events")
...
@dataclass(frozen=True, slots=True)
class HistoricalV25EvidenceReadiness:
    """Machine-readable prerequisites for a fully locked SH25 historical reference."""
    ...
    def __post_init__(self) -> None:
        if not self.symbol.strip():
            raise ValueError("symbol is required")
        if self.trading_authorized or self.live_trading_enabled:
            raise ValueError("historical SH25 readiness cannot authorize trading")
        if self.ready_for_locked_reference and self.locked_reference_id is None:
            raise ValueError("ready SH25 historical evidence requires a locked reference id")
...
def assess_forward_v25_evidence_readiness(
    *,
...
) -> ForwardV25EvidenceReadiness:
    """Report exact missing/invalid inputs before verified-input SH25 forward replay."""
    ...
    return ForwardV25EvidenceReadiness(
        symbol=normalized_symbol,
...
def assess_historical_v25_evidence_readiness(
    *,
    symbol: str,
    market_csv: str | None = None,
    ...
    """Inventory exact SH25 historical proof artifacts including shadow_forward_start."""
    normalized_symbol = symbol.strip().upper()
    if not normalized_symbol:
        raise ValueError("symbol is required")
    ...
    if not blockers:
        try:
            locked = build_historical_v25_reference(
                symbol=normalized_symbol,
            ...
            if (
                "parameter" in detail.lower()
                or "PROCESS_ORDERS_ON_CLOSE" in detail
                or "SHADOW_FORWARD_START" in detail
            ):
                manifest_state = EvidenceArtifactState.INVALID
        else:
            locked_reference_id = locked.reference_id

    return HistoricalV25EvidenceReadiness(
        symbol=normalized_symbol,
        market_evidence_state=market_state,
