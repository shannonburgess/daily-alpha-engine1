...
account state, billing-event idempotency, entitlement decisions, privileged override
audit evidence, and launch readiness without charging customers or enabling brokerage
execution.
"""
...
def apply_billing_event(
    ...
    if event.event_type == BillingEventType.SUBSCRIPTION_STARTED:
        ...
        if tier_id is None:
            return BillingApplyResult(
                projection=projection,
                disposition="INVALID_STATE_IGNORED",
            )
        state = SubscriptionState.ACTIVE
...
@dataclass(frozen=True)
class PrivilegedEntitlementOverrideAudit:
    """Audit evidence for a future authenticated support/admin override path.

    This record does not itself grant or revoke access.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not all(
            ...
            raise ValueError("override audit identity, actor, entitlement and reason are required")
        if self.expires_at is not None:
            expires = datetime.fromisoformat(self.expires_at)
            if expires <= occurred:
                raise ValueError("override expiry must be after occurred_at")


@dataclass(frozen=True)
class CommercialBetaReadiness:
    reproducible_research_history: bool
    research_provenance_replay: bool
    performance_basis_separation: bool
    performance_methodology_contract: bool
    customer_visible_claim_gate: bool
    delivery_monitoring: bool
    entitlement_isolation_tests: bool
    billing_reconciliation: bool
    privileged_override_audit: bool
    external_legal_compliance_review: bool
    terms_privacy_support_complete: bool
    customer_data_retention_controls: bool
    ...
    def missing_gates(self) -> tuple[str, ...]:
        required = {
            ...
            "RESEARCH_PROVENANCE_REPLAY": self.research_provenance_replay,
            "PERFORMANCE_BASIS_SEPARATION": self.performance_basis_separation,
            "PERFORMANCE_METHODOLOGY_CONTRACT": self.performance_methodology_contract,
            "CUSTOMER_VISIBLE_CLAIM_GATE": self.customer_visible_claim_gate,
            ...
            "PRIVILEGED_OVERRIDE_AUDIT": self.privileged_override_audit,
            "EXTERNAL_LEGAL_COMPLIANCE_REVIEW": self.external_legal_compliance_review,
            "TERMS_PRIVACY_SUPPORT_COMPLETE": self.terms_privacy_support_complete,
            "CUSTOMER_DATA_RETENTION_CONTROLS": self.customer_data_retention_controls,
