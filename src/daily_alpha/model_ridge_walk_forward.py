"""Research-only ridge challenger: TRAIN fit -> VALIDATION select -> untouched TEST."""
...
from .model_linear_baseline import LinearBaselineFit, fit_linear_baseline
...
SIGNAL_THRESHOLD_KEY = "signal_threshold"


@dataclass(frozen=True, slots=True)
class RidgeWalkForwardCandidate:
    ...
    def __post_init__(self) -> None:
        if self.specification.specification_id != self.fitted.model.specification_id:
            raise ModelTrainingError("RIDGE_CANDIDATE_SPECIFICATION_FIT_MISMATCH")
        if self.validation_trial.artifact != self.fitted.artifact:
            raise ModelTrainingError("RIDGE_CANDIDATE_VALIDATION_ARTIFACT_MISMATCH")
...
class RidgeWalkForwardResult:
    dataset_id: str
    fold_id: str
    candidates: tuple[RidgeWalkForwardCandidate, ...]
    ...
    def __post_init__(self) -> None:
        if not self.candidates:
            raise ModelTrainingError("RIDGE_WALK_FORWARD_CANDIDATES_REQUIRED")
        if self.selection.dataset_id != self.dataset_id:
            raise ModelTrainingError("RIDGE_SELECTION_DATASET_MISMATCH")
        if self.selection.fold_id != self.fold_id:
            raise ModelTrainingError("RIDGE_SELECTION_FOLD_MISMATCH")
        if self.final_test.dataset_id != self.dataset_id:
            raise ModelTrainingError("RIDGE_FINAL_TEST_DATASET_MISMATCH")
        if self.final_test.fold_id != self.fold_id:
            raise ModelTrainingError("RIDGE_FINAL_TEST_FOLD_MISMATCH")
        ...
        if self.selection.selected_specification_id not in candidate_spec_ids:
            raise ModelTrainingError("RIDGE_SELECTED_SPECIFICATION_NOT_FIT")
        if any(
            (
            ...
            raise ModelTrainingError("RIDGE_WALK_FORWARD_CANNOT_AUTHORIZE_TRADING")

    @property
    def selected_candidate(self) -> RidgeWalkForwardCandidate:
        return next(
            candidate
...
def run_ridge_walk_forward_challenger(
    *,
    ...
    """Run a complete leak-proof ridge challenger over one frozen walk-forward fold."""
    if not specifications:
        raise ModelTrainingError("RIDGE_SPECIFICATIONS_REQUIRED")
    if len({item.specification_id for item in specifications}) != len(specifications):
        raise ModelTrainingError("RIDGE_SPECIFICATIONS_MUST_BE_UNIQUE")

    validation_examples = _examples_for_ids(dataset, fold.validation_example_ids)
    candidates: list[RidgeWalkForwardCandidate] = []
    for specification in specifications:
        if specification.family is not ModelFamily.LINEAR_SCORE:
            raise ModelTrainingError("RIDGE_WALK_FORWARD_REQUIRES_LINEAR_SCORE_FAMILY")
        threshold = _signal_threshold(specification)
        fitted = fit_linear_baseline(
            dataset=dataset,
            fold=fold,
    ...
    test_strategy_r = _strategy_realized_r(
        ...
        threshold=_signal_threshold(selected.specification),
    )
...
def _signal_threshold(specification: ModelSpecification) -> float:
    parameters = dict(specification.hyperparameters)
    raw = parameters.get(SIGNAL_THRESHOLD_KEY)
    if raw is None:
        raise ModelTrainingError("RIDGE_SIGNAL_THRESHOLD_REQUIRED")
    try:
        threshold = float(raw)
    except ValueError as exc:
        raise ModelTrainingError("RIDGE_SIGNAL_THRESHOLD_INVALID") from exc
    if not math.isfinite(threshold):
        raise ModelTrainingError("RIDGE_SIGNAL_THRESHOLD_MUST_BE_FINITE")
    return threshold
...
def _strategy_realized_r(
    *,
    ...
    return tuple(
        example.realized_r if fitted.model.predict(example) > threshold else 0.0
        for example in examples
    )


__all__ = [
    "SIGNAL_THRESHOLD_KEY",
    "RidgeWalkForwardCandidate",
    "RidgeWalkForwardResult",
    "run_ridge_walk_forward_challenger",
]
