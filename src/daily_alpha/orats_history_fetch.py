...
def fetch_daily_earnings_rows(
    ...
    *,
    ...
    The daily and earnings endpoints can independently require the compatibility
    route. The final backtest adapter therefore must not collapse them into one
    shared source label. Existing payload `source` fields are overwritten by the
    transport provenance so a payload cannot spoof its origin.
    """
