...
def account_id_for_ingress(ingress: Mapping[str, Any]) -> str:
    ...
    default_account = (
        os.getenv("DAILY_ALPHA_PAPER_ACCOUNT_ID", "").strip() or "paper-staging"
    )
...
class ShadowRoutedPinePaperExecutor:
    """Execute an explicitly tagged model against its own durable paper ledger."""
    ...
    def execute(
        ...
        *,
        ...
        execution = ReconciledAwsPinePaperExecutor(**kwargs).execute(ingress, now=now)
