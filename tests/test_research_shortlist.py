...
def test_trump_policy_bonus_can_prioritize_research_but_not_authorize_trade(tmp_path):
    previous = write_csv(
        tmp_path / "OVTLYR_2026-08-13.csv",
        [
            "AAA,Hold,2026-08-13,Technology,Software,Up,Rising,Yes,100,1000000",
    ...
    assert source.requested == ("BBB",)
    by_symbol = {item.symbol: item for item in result.items}
    assert by_symbol["BBB"].trump_policy_bonus == 3.0
    assert by_symbol["BBB"].trump_policy_rank == 1
    assert by_symbol["AAA"].trump_policy_bonus == 0.0
    assert result.summary["trump_policy_matched_candidates"] == 1
...
def test_write_outputs_includes_newsletter_rotation_and_confirmation_files(tmp_path):
    previous = write_csv(
        tmp_path / "OVTLYR_2026-08-13.csv",
        ["AAA,Hold,2026-08-13,Technology,Software,Up,Rising,Yes,100,1000000"],
    )
