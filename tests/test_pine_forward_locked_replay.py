...
from daily_alpha.pine_forward_locked_replay import evaluate_locked_forward_v24_reference
...
def _deployment(events=()) -> ForwardParityDeploymentEvidence:
    return ForwardParityDeploymentEvidence(
        ...
        sh24=_book("PAPER_SHADOW_V24", events),
        sh25=_book("PAPER_SHADOW_V25"),
    )


def test_locked_forward_builder_replays_exact_market_and_parameter_artifacts() -> None:
    market_csv = _market_csv()
    locked = evaluate_locked_forward_v24_reference(
        deployment=_deployment(),
        symbol="DINO",
        market_csv=market_csv,
        parameter_manifest_json=_manifest(),
        market_source="POINT_IN_TIME_DAILY_OHLCV",
        market_revision="DINO-2026-08-23-v1",
        python_engine_revision="pine_v24_parity.py@test",
    )
    ...
    assert locked.replay_provenance.deployment_commit_sha == "b" * 40


def test_locked_forward_builder_requires_receipt_event_bar_in_market_evidence() -> None:
    missing_bar = BASE + timedelta(days=60)
    with pytest.raises(ValueError, match="absent from locked market evidence"):
        evaluate_locked_forward_v24_reference(
...
def test_locked_forward_builder_rejects_cross_symbol_genuine_receipt_event() -> None:
    event = _event(BASE + timedelta(days=34))
    fields = dict(event.fields)
    fields["symbol"] = "MU"
    cross_symbol = ForwardPersistedEventEvidence(
        account_id="PAPER_SHADOW_V24",
        signal_id=event.signal_id,
        fields=tuple(sorted(fields.items())),
    )
    with pytest.raises(ValueError, match="outside the requested symbol"):
        evaluate_locked_forward_v24_reference(
            deployment=_deployment((cross_symbol,)),
            symbol="DINO",
            market_csv=_market_csv(),
            parameter_manifest_json=_manifest(),
            market_source="POINT_IN_TIME_DAILY_OHLCV",
...
def test_locked_forward_builder_requires_process_orders_on_close_true() -> None:
